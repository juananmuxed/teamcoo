<template>
  <v-row>
    ...
    <v-col
      ...
      sm="6"
      class="pa-2"
      v-if="isLogged() && this.$route.path != '/dashboard'"
    >
      <v-btn
        color="warning"
        block
    ...
    <v-col cols="12" sm="6" class="pa-2" v-if="!isLogged()">
      <v-dialog v-model="dialogs.login" max-width="500">
        <template v-slot:activator="{ on }">
          <v-btn color="primary" block height="160" v-on="on" class="display-1"
            >Login</v-btn
          >
        </template>
