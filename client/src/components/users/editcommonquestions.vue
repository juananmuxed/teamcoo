<template>
  <v-card max-width="650" class="mx-auto pa-2">
    <v-card-title class="headline font-weight-medium text-uppercase">Edit Common Questions from "{{ loadedUser.username }}"</v-card-title>
    <v-card-text>
        <v-form ref="form" v-model="valid">
            <v-row v-for="( question, index) in common" v-bind:key="index" class="px-3">
            <v-col cols="12" md="3">
        ...
        <v-slide-y-transition origin="center center">
            <v-btn fab right small top absolute color="primary" @click="saveCommonQuestions({idUser:loadedUser._id, answers:answers})" v-show="valid" class="mt-8">
...
export default {
    ...
    async created() {
        this.common = this.commonQuestions;
        for (let i = 0; i < this.common.length; i++) {
            ...
            let obj = {
                ...
                answer: this.loadedUser.commonquestions.some(q => q._id == this.common[i]._id) ? this.loadedUser.commonquestions[i].answer : answer
            };
