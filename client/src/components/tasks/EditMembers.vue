<template>
    <v-card
        ...
        class="mx-auto"
    >
        <v-card-title class="headline font-weight-medium text-uppercase">Edit Members from "{{ searchedTask.name }}"</v-card-title>
        <v-card-text>
            <v-row>
            ...
            <v-slide-y-transition origin="center center">
                <v-btn fab right small top absolute color="info" @click="loadMembers({members:searchedTask.members})" v-show="!editedMembers()" class="mt-8">
...
export default {
    computed: {
        ...mapState({
            ...
            editmemberform: state => state.tasks.editmemberform,
